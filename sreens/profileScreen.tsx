...
import { Datepicker, Icon, Input, Text, Button, Modal, Card } from "@ui-kitten/components";
...
export const ProfileScreen = () => {
  ...
  return (
    <View style={styles.container}>
      <Text category="h4" style={styles.label}>Профиль</Text>
      <Input
        value={model?.lastName}
        label='Фамилия'
        onChangeText={lastName => setModel(model ? { ...model, lastName } : model)}
      />
      <Input
        value={model?.firstName}
        label='Имя'
        onChangeText={firstName => setModel(model ? { ...model, firstName } : model)}
        style={styles.field}
      />
      <Input
        value={model?.patronymic}
        label='Отчество'
        onChangeText={patronymic => setModel(model ? { ...model, patronymic } : model)}
        style={styles.field}
      />
      <Input
        value={model?.specialty}
        label='Специальность'
        onChangeText={specialty => setModel(model ? { ...model, specialty } : model)}
        style={styles.field}
      />
      <Datepicker
        label='Дата рождения'
        date={model?.birthday}
        onSelect={birthday => setModel(model ? { ...model, birthday } : model)}
