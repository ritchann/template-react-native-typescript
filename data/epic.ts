...
import { getData, setIncident, setSiteEvent } from "./api";
...
const setSiteEventEpic = (action$: ActionsObservable<any>) => {
  return action$.pipe(
    ...
    mergeMap(action => from(setSiteEvent((action as any).data)).pipe(
      ignoreElements())
))};
...
const setIncidentEpic = (action$: ActionsObservable<any>) => {
  return action$.pipe(
    ...
    mergeMap(action => from(setIncident((action as any).data)).pipe(
      ignoreElements())
))};

export const epic = combineEpics(getDataEpic, setSiteEventEpic, setIncidentEpic);
